...
(function ($) {
    ...
    $.fn.makeEditable = function (options) {
        ...
        var sNewCellValue, sNewCellDislayValue;
        ...
        function fnApplyEditable(aoNodes) {
            ...
            var oDefaultEditableSettings = {
                ...
                "onsubmit": function (settings, original) {
                    ...
                    $(original).data("sOldValue", sOldValue);
                    sNewCellValue = null;
                    sNewCellDisplayValue = null;
                    iDisplayStart = fnGetDisplayStart();

                    if (settings.type == "text" || settings.type == "select" || settings.type == "textarea" || settings.type == "checkbox" || settings.type == "datetimepicker" || settings.type == "formattedText" || settings.type == "autocomplete") {

                        var input = null;

                        if (settings.type == "autocomplete") {
                            input = $("input.value", this);
                            sNewCellValue = $(input).val();
                        } else {
                            input = $("input,select,textarea,checkbox,datetimepicker,formattedText", this);
                            sNewCellValue = $("input,select,textarea,checkbox,datetimepicker,formattedText", $(this)).val();
                        }
                        ...
                        if (input.length > 0) {
                            ...
                            if (oEditElement.nodeName.toLowerCase() == "select" || oEditElement.tagName.toLowerCase() == "select")
                                ...
                                sNewCellDisplayValue = $(input).is(":checked");
                                $("input,select,textarea,checkbox,datetimepicker,formattedText", $(this)).parents('td').data('value', sNewCellDisplayValue);
                            } else if (settings.type == "autocomplete") {
                                sNewCellDisplayValue = $(input).prev(".ui-autocomplete-input").val();

                                if (!$(input).val() || $(input).val() == "") {
                                    sNewCellDisplayValue = "";
                                }

                                $(input).parents("td").data("value", sNewCellDisplayValue);
                            } else {
                                sNewCellDisplayValue = sNewCellValue;
                                $("input,select,textarea,checkbox,datetimepicker,formattedText", $(this)).parents('td').data('value', sNewCellValue);
                            }
                        }
                ...
                "submitdata": function (value, settings) {
                    ...
                    if (settings.type === "autocomplete") {
                        updateData["value"] = $(this).children("form").children(".value").val();
                        var displayValue = $(this).html();
                        updateData["displayValue"] = displayValue.toUpperCase() == 'NULL' ? '' : displayValue;
                    }
                ...
                "callback": function (sValue, settings) {
                    ...
                    var sOldValue = $(this).data('sOldValue');
                    ...
                    if (sValue.indexOf(properties.sFailureResponsePrefix) > -1) {
                        oTable.fnUpdate(sOldValue, aPos[0], aPos[2], false, false);
                        ...
                        if (properties.sSuccessResponse == "IGNORE" ||
                            ...
                            (sNewCellValue == null) || (sNewCellValue == sValue) ||
                             properties.fnIsSuccess(sValue)) {
                            //properties.sSuccessResponse == sValue) {
                            if (sNewCellDisplayValue == null || sNewCellDisplayValue.toString().toLowerCase() == 'null') {
                                //sNewCellDisplayValue = sValue;
                                //oTable.fnUpdate(sOldValue, aPos[0], aPos[2], bRefreshTable);
                                oTable.fnUpdate('', aPos[0], aPos[2], false, false);
                            } else {

                                var visibleLocation = locateVisibleColumn(aPos[2]);
                                var displayValue = $(oTable).find('tbody tr').eq(aPos[0]).find('td').eq(visibleLocation).data('value');

                                if (settings.FormatString) {
                                    displayValue = Globalize.parseFloat(displayValue.toString()).toString();
                                }

                                oTable.fnUpdate(displayValue, aPos[0], aPos[2], false, false);
                            }
                            $("td.last-updated-cell", oTable).removeClass("last-updated-cell");
                            $(this).addClass("last-updated-cell");
                            status = "success";
        ...
        function locateVisibleColumn(visibleIndex) {

            var index = 0;
            for (var i = 0; i <= visibleIndex; i++) {
                if (oTable.api().columns().column(i).visible()) {
                    index++;
                }
            }
            return index - 1;
        ...
        */
        ...
        function _fnIsSuccess(response) {
            return (properties.sSuccessResponse == response);
        }
